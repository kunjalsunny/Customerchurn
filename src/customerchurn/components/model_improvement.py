...
from sklearn.metrics import roc_auc_score,average_precision_score,recall_score,f1_score,precision_score,confusion_matrix
...
class ModelImprovement:
    ...
    def _get_candidates(self):
        ...
        for alpha in [1e-4,1e-3,1e-2]:
            candidates.append((
                ...
                SGDClassifier(loss="log_loss",alpha=alpha,class_weight='balanced',random_state=self.random_state)
    ...
    def initiate_model_improvement(self,X_train_path,X_test_path,y_train_path,y_test_path):
        try:
            logging.info("=======Model Improvement is running========")
            ...
            # If X is sparse, it still has shape[0]
            ...
            if n_train != y_train.shape[0]:
                raise ValueError(f"Mismatch: X_train has {n_train} rows but y_train has {y_train.shape[0]} labels. "
                                f"Check that you are loading/saving the correct files and not mixing train/test.")
            if n_test != y_test.shape[0]:
                raise ValueError(f"Mismatch: X_test has {n_test} rows but y_test has {y_test.shape[0]} labels.")



            logging.info(f"Loaded arrays. Train: {getattr(X_train,'shape',None)} | Test: {getattr(X_test,'shape',None)}")
            ...
            for name, est in self._get_candidates():
                ...
                # choose best by PR-AUC (often better for churn than ROC-AUC)
